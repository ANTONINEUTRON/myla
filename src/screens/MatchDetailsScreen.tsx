...
// import { duelduckService } from '../services/duelduck';
...
//   // ─── Local State ──────────────────────────────────────────────────
//   const { walletAddress, signMessage, signAndSendTransaction, balance } = useWallet();
...
//   // ─── Initialize Simulator Histories (Only for non-live matches) ───
//   useEffect(() => {
//     if (!feedMatch || feedMatch.status === 'live') return;
...
//   // ─── Fetch real scores from TxODDS API ──────────────────────────────
//   const fetchLiveScores = useCallback(async () => {
//     try {
//       const scores = await txoddsService.getScores(matchId);
...
//       } else {
//         const startMs = feedMatch ? new Date(feedMatch.startTime).getTime() : Date.now();
...
//     if (feedMatch.status === 'live') {
//       fetchLiveScores();
//       const interval = setInterval(fetchLiveScores, 12_000);
//       return () => clearInterval(interval);
//     } else if (feedMatch.status === 'finished') {
//       fetchLiveScores();
//     }
//   }, [feedMatch, fetchLiveScores]);
...
//   // ─── Confirm & Place Real On-Chain Trade via DuelDuck ──────────────
//   const handleExecuteTrade = async () => {
//     if (!selection || !feedMatch) return;
...
//     try {
//       // 1. Authenticate with DuelDuck if not authenticated
//       if (!duelduckService.isAuthenticated()) {
//         const message = 'Sign in to DuelDuck';
//         const signature = await signMessage(message);
//         await duelduckService.signInWithWallet(walletAddress, signature);
//       }

//       // 2. Prepare duel config
//       const directionLabel = tradeDirection === 'hi' ? 'Over' : 'Under';
//       const question = `Will ${asset} be ${directionLabel} ${selection.strikeLevel} at Minute ${selection.strikeMinute} in ${feedMatch.homeTeam} vs ${feedMatch.awayTeam}?`;
      
//       const deadlineDate = new Date(new Date(feedMatch.startTime).getTime() + selection.strikeMinute * 60 * 1000);

//       const duelConfig = {
//         answer: tradeDirection === 'hi' ? 0 : 1,
//         commission_rate: 5, // 5% commission
//         deadline: deadlineDate.toISOString(),
//         duel_info: {
//           matchId: feedMatch.id,
//           asset,
//           strikeMinute: selection.strikeMinute.toString(),
//           strikeLevel: selection.strikeLevel.toString(),
//           homeTeam: feedMatch.homeTeam,
//           awayTeam: feedMatch.awayTeam,
//         },
//         duel_price: stake,
...
//         price_type: 'FIXED',
//         question,
//         source_of_truth: 'TxODDS API Live Feed Data',
//         symbol: 'SOL',
//       };

//       // 3. Get prepared transaction base64 from DuelDuck
//       const txBase64 = await duelduckService.getCreateDuelTransaction(duelConfig);

//       // 4. Sign and broadcast transaction via MWA / local wallet
//       const txHash = await signAndSendTransaction(txBase64);

//       // 5. Confirm creation in DuelDuck backend
//       const result = await duelduckService.confirmCreateDuel(duelConfig, txHash);

//       // 6. Record local position
//       const oddsRatio = tradeDirection === 'hi' ? odds.hi : odds.lo;
//       const newPosition: OptionPosition = {
//         id: result?.duel?.id || Math.random().toString(36).substring(2, 11),
//         asset,
...
//       setSelection(null);
//       Alert.alert('Prediction Created', 'Your micro-prediction has been placed on-chain via DuelDuck.');
//     } catch (err: any) {
//       console.error('[MatchDetailsScreen] handleExecuteTrade failed:', err);
...
//                   <Text style={styles.ftLabelDetail}>FT</Text>
//                 )}
//               </View>
//             )}
...
//   ftLabelDetail: { color: THEME.colors.primary.DEFAULT, fontSize: 11, fontWeight: '800', letterSpacing: 1, marginTop: 4, backgroundColor: THEME.colors.primary.glow, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
